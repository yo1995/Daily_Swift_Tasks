...
class Decoders {
    ...
    static private func initialize() {
        dispatch_once(&once) {
            ...
            // Decoder for [JournalEntry]
            Decoders.addDecoder(clazz: [JournalEntry].self) { (source: AnyObject) -> [JournalEntry] in
                return Decoders.decode(clazz: [JournalEntry].self, source: source)
            }
            // Decoder for JournalEntry
            Decoders.addDecoder(clazz: JournalEntry.self) { (source: AnyObject) -> JournalEntry in
                let sourceDictionary = source as! [NSObject:AnyObject]
                let instance = JournalEntry()
                instance.id = Decoders.decodeOptional(clazz: String.self, source: sourceDictionary["id"])
                instance.emoji = Decoders.decodeOptional(clazz: String.self, source: sourceDictionary["emoji"])
                instance.date = Decoders.decodeOptional(clazz: Double.self, source: sourceDictionary["date"])
                return instance
            }
